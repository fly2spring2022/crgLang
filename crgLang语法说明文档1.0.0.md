...
## 概述
**crgLang** 是一种专为 AI Prompt 生成而设计的脚本语言，面向自然语言处理中的大语言模型（如 GPT-4）的交互和应用。由于 crgLang 的解析器和编译器本身就是 AI 模型，所以它具备极大的灵活性，能够处理自然语言中的复杂表达，简化了代码执行的过程。
...
每个 crgLang 脚本的开头包含一组元数据声明，用于定义脚本的基本信息和运行所需的模型环境。
...
#### 说明：
- **name**: 脚本名称，用于标识 crgLang 项目。
- **version**: 脚本版本号，遵循语义化版本规范（如：1.0.0）。
- **author**: 作者信息，可以填写个人或团队名称。
- **date**: 脚本创建或修改的日期。
- **supported_models**: 脚本支持的 AI 大模型列表，指明该脚本兼容的模型（如 GPT-4、ChatGPT）。

---

## 模块结构

crgLang 是一个**任务导向-模块化**脚本语言。模块是脚本的基本执行单元。每个模块接受输入，处理逻辑，返回输出结果。任务导向是指所有模块的首行都必须有描述该模块任务或功能的声明指令。
...
`init()` 模块是脚本程序的起点，主要功能是展示脚本的功能说明，提示用户输入，并提供初始界面。初始化模块为用户提供明确的交互内容和指引。
...
`main()` 模块负责管理脚本程序的主要逻辑，完成生成 AI Prompt 的主要任务。在此过程中，主模块会调用其他子模块来完成特定任务。主模块通常是整个程序的核心逻辑，负责串联并执行所有步骤。
...
```plaintext
def main() {
    # 脚本主程序，执行角色设定、用户输入解析并生成最终 prompt
...
`output()` 模块用于处理脚本程序的最终输出内容，通常将生成的 prompt 输出到控制台或其他指定目标。同样的，输出模块也仅是一个普通模块，鉴于它在AI prompt中的重要位置，所以单独提取出来说明。
